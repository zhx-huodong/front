...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        type: [{ required: true, message: "请选择管理权限", trigger: "change" }]
      },
      areaList: [
  ...
  methods: {
    ...
    async submitForm() {
      ...
      //如果选了区域管理员和市管理员
      if (
        this.ruleForm.type.indexOf(4) > -1 &&
        this.ruleForm.type.indexOf(3) > -1 &&
        this.area.length > 0
      ) {
        params.area_id = this.area;
        if (params.area_id.indexOf(77) == -1) {
      ...
      if (
        ...
        if (this.area.indexOf(77) > -1) {
          this.area.splice(this.area.indexOf(77), 1);
        }
        params.area_id = this.area;
      }
      //如果没选区管理员和市管理员
    ...
    //   console.log("area===", this.area, "params===", params.area_id);
    //   return;
      if (this.id != undefined) {
        params.id = this.id;
